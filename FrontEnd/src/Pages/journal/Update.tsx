...
function UpdateJournal() {
    ...
    const [JournalForm,SetForm] = useState<Partial<Journal>>({
        mood:"",
        energy:"",
        activities:[],
        title:"",
 ...
 const [JournalFormOg,SetFormOg] = useState<Partial<Journal>>()
  ...
  function getUpdated(){
    const Updated = {}
    Object.keys(JournalFormOg).forEach((v)=>{
        const Ogdata = JSON.stringify(JournalFormOg[v])
        const Curntdata = JSON.stringify(JournalForm[v])
        if(Ogdata !== Curntdata){
         Updated[v] = JSON.parse(Curntdata)
        }
    })
    return Updated
  }
   ...
   async function FetchOne() {
    try {
        const {_id,user_id,createdAt,...Data} = await JournalSercice.FindOne(id as string) as Journal
            SetForm(structuredClone(Data))
            SetFormOg(structuredClone(Data))
    } catch (error) {
        console.log(error);
    }
   }
